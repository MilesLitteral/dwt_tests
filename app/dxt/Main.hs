...
log_symbol :: IsString p => LogLevel -> p
log_symbol sym =
  case sym of 
      LOG_INFO    -> "ℹ️" 
      LOG_MESSAGE -> "💬" 
...
ez8 :: Image PixelRGB8 -> Image PixelRGBF
ez8 = pixelMap $ \(PixelRGB8 r g b ) ->  promotePixel $ PixelRGBF (promotePixel  r) (promotePixel g) (promotePixel  b)


getImage :: Either a DynamicImage -> IO (Image PixelRGB8)
getImage ids = do
    case ids of
      Left err    -> error "\x1b[31m╚(🈲)Not An Image"
      Right image -> return $ convertRGB8 image

--putStrLn $ show (dwt 5 (haar) (wp_interleave) dt)

main  ::  IO ()
main = do  
    orderedMessage $ LogMessage LOG_HEAD LOG_ZONE ("Booting DWT")
    ids <- readImage "test.png"
    sd  <- getImage ids
    orderedMessage $ LogMessage LOG_BODY LOG_ZONE ("Image to Floats")
    let rx = ez8 sd
    let dt =  Data.Vector.Storable.toList $ (imageData rx)
    let xt =  Data.Vector.Storable.fromList $ (dwt 25 (haar) (wp_interleave) dt)
    orderedMessage $ LogMessage LOG_BODY LOG_ZONE ("Ran DWT on [Floats]")
    let img :: DynamicImage = (ImageRGBF (Image 3120 3920 xt))
    orderedMessage $ LogMessage LOG_BODY LOG_ZONE ("Compiling Results to PNG")
    orderedMessage $ LogMessage LOG_TAIL LOG_ZONE ("Saving DWT Image")
    savePngImage "results.png" img
